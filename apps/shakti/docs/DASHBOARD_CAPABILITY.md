...
The reusable `DashboardProvider`/`useDashboardConfig` context now lives in `packages/dashboard-platform/src/config/`; SHAKTI's concrete zone map and default values live in `src/types/dashboard.types.ts` and `src/config/dashboard.config.ts`, and are passed into the provider as `defaultConfig`.
