...
| Hook | Service | Queries |
|------|---------|---------|
| `useQueries.ts` | Control Plane | Runtime metrics, health, telemetry, alerts, workflows, evidence |
| `useBucketQueries.ts` | Bucket | Repository registry, build registry, migration queue |
| `useNiyantranQueries.ts` | Niyantran | Employee execution, engineering capacity, delivery intel, capabilities |
| `usePranaQueries.ts` | Prana | Runtime sessions |
| `useInsightFlowQueries.ts` | InsightFlow | Decision intelligence |
...
| 5 | **Routing** | Currently single-page (`App.tsx → Dashboard`); `react-router-dom` is installed but not configured for multi-page navigation | Low |
...
3. **Configure routing** — Wire up `react-router-dom` for multi-page navigation using `NavigationEngine`
